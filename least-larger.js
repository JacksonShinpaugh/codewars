...
// number that is larger than the element at the given index, or -1 if there is no such index 
...
function leastLarger(a,i) {
    const num = a[i]
    const largerNums = a.filter(n => n > num)
    if (largerNums.length === 0) {
        return -1
    } else {
        return a.indexOf(Math.min(...largerNums))
    }
}
